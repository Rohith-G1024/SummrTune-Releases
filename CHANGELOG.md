...
Each release describes what the app **does**, rather than what changed — you
should not have to read the previous entry to understand this one.

---

## 1.0.0 — 2026-08-23

The first full release. **This is what the app does**, not what changed since
0.9.0 — 0.9.0 was a pre-release that went to two people, and a version anybody
can download should describe itself rather than assume you saw the one before it.
...
- A take starts **at the playhead**, not at zero, and everything else plays while
  you sing.
- **Punch in** over a stretch of an existing track without making a new one — the
  rest of the take is untouched, and the seam is crossfaded.
...
- Listen back to a take before you keep it. Nothing joins the project until you
  say so.
- **Duplicate a track** to try two versions of an idea side by side.
- **Import from the timeline** as well as from the header — a `+` row sits at the
...
- **Pitch correction** — key-locked, from a gentle repair through to the hard
  tuned sound, with a keyboard for picking notes by hand. The keys play their
  note, in the song's own tuning rather than concert pitch.
...
- **Convert a stereo track to mono**, which unlocks the effects that need a
  single voice. It asks first and cannot be undone.
...
- **Back up a project to a file**, and restore it later. A backup is a single
  file you choose the location for; restoring adds it as a new project and never
  touches what you already have.
- Back up from a project's ⋯ menu; restore from **Settings → Storage**.
- **Free up space** returns the room taken by the originals an effect kept, on
  one project or on all of them.

### Around the edges

- **Nine colour schemes**, each in light and dark. Track colours follow the
  scheme — every family has its own twelve, and colour can be set on any track.
...
- An **update notice**: once a day the app asks the release page whether a newer
  ...
  sends or asks for, it never downloads or installs anything — the notice opens
  the download in your browser and you decide — and it can be switched off from
  the notice itself or from Settings. **Check for updates** in Settings asks on
  demand, and is the one place a failed check says so.
...
- Uninstalling deletes every project. Back up or export anything you want to
  keep.
